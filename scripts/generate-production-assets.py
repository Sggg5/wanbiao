...
def dial(kind):
    ...
    # Brushing is deliberately below the threshold where it reads as drawn spokes.
    ray_profiles = {
        'blue': (180, 1, 3),
        'black': (120, 1, 2),
        'silver': (0, 0, 0),
    }
    ray_count, low_alpha, high_alpha = ray_profiles[kind]
    for index in range(ray_count):
        angle = math.radians(index * (360 / ray_count))
        alpha = low_alpha if index % 2 else high_alpha
        d.line((
            CX, CY,
            CX + math.cos(angle) * 302, CY + math.sin(angle) * 302,
    ...
    if kind == 'silver':
        ...
        hd.ellipse((CX - 315, CY - 290, CX + 120, CY + 195), fill=(255, 252, 245, 27))
        ...
        highlight = Image.new('RGBA', CANVAS, (0, 0, 0, 0))
        hd = ImageDraw.Draw(highlight, 'RGBA')
        hd.ellipse((CX - 245, CY - 350, CX + 115, CY + 20), fill=(255, 255, 255, 12))
        highlight = highlight.filter(ImageFilter.GaussianBlur(78))
        highlight.putalpha(ImageChops.multiply(highlight.getchannel('A'), circle_mask))
        image.alpha_composite(highlight)
    else:
        highlight = Image.new('RGBA', CANVAS, (0, 0, 0, 0))
        hd = ImageDraw.Draw(highlight, 'RGBA')
        hd.polygon(((365, 455), (515, 390), (845, 770), (700, 845)), fill=(180, 220, 255, 11))
